...
export const getWordAndExercise: (wordExerciseList:ExerciseWord[], size:number) => ExerciseListWordList = (wordExerciseList:ExerciseWord[], size:number) : ExerciseListWordList => {
    ...
    let filteredWordList: WordCount[] = wordLevelList.filter((e:WordCount): boolean => e.count === 1)
    if (filteredWordList.length < size) { //Si tous les mots de niveau 1 ne dépasse pas le nombre voulu
        filteredWordList = [...filteredWordList, ...wordLevelList.filter((e:WordCount): boolean => e.count === 2)] //On ajoute les nombres de niveau 2
    }
    ...
    const selectedWordList: ExerciseWord[] = [...wordExerciseList.filter((e:ExerciseWord):boolean => filteredWordList.some((a:WordCount): boolean => a.word_list_id === e.word_list_id))]

    //Récupère le groupe de la liste d'exercice n°1, ayant le plus de valeur
    const selectedExercise1: ExerciseCount = countPowerByExercise(selectedWordList, listExercise1).reduce((acc:ExerciseCount, obj:ExerciseCount):ExerciseCount => obj.totalPower > acc.totalPower ? obj : acc)

    //Récupère les informations des WordExercises avec les id de la liste d'exercice choisi
    const selectedWordListByEx1: ExerciseWord[] = wordExerciseList.filter((e:ExerciseWord):boolean => {
        return selectedExercise1.words.some((w:WordPower) :boolean => {
            return w.word_list_id === e.word_list_id
        })
    })

    //Récupère le groupe de la liste d'exercice n°2, ayant le plus de valeur avec les mots choisi depuis l'exercice 1
    const selectedExercise2: ExerciseCount = countPowerByExercise(selectedWordListByEx1, listExercise2).reduce((acc:ExerciseCount, obj:ExerciseCount):ExerciseCount => obj.totalPower > acc.totalPower ? obj : acc)
    ...
    const allWordSelected: string[] = selectedExercise1.words.map((w:WordPower):string => w.word_list_id)
...
const countValidationByWordListAndMethod: (data: ExerciseWord[]) => ValidationCount[] = (data: ExerciseWord[]) : ValidationCount[] => {
    return data.reduce((acc: ValidationCount[], curr: ExerciseWord):ValidationCount[] => {
        if (!curr.validate) { //Si validate est False ou Null
            const existingCount: ValidationCount | undefined = acc.find((item:ValidationCount):boolean => // On cherche si une validation pour ce mot et cette méthode a déjà été créé
...
const groupByWordList: (data: ValidationCount[]) => WordCount[] = (data: ValidationCount[]) : WordCount[] => {
    return data.reduce(
        (acc:WordCount[] , item :ValidationCount): WordCount[] => {
...
const countPowerByExercise: (wordList:ExerciseWord[], exerciseList:ExerciseCount[]) => ExerciseCount[] = (wordList:ExerciseWord[], exerciseList:ExerciseCount[]) : ExerciseCount[] => {
    wordList.map((exerciseWord:ExerciseWord):void => { //Pour chaque ExerciceWord
        exerciseList.map((exerciseCount:ExerciseCount): void => { // Pour chaque groupe d'exercice
            if((exerciseWord.validate === false || exerciseWord.validate === null) && exerciseCount.listExercise.includes(exerciseWord.exercise_id)){ // On vérifie si l'exercice est False ou Null et s'il se trouve dans le groupe d'exercice
                let index:number = exerciseCount.words.findIndex((a:WordPower):boolean => a.word_list_id === exerciseWord.word_list_id)  //On cherche l'index du mot dans la liste de mot de l'exercice
